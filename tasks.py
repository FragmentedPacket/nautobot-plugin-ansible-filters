...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_ANSIBLE_FILTERS_xxx
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.ansible_filters.compose_http_timeout,
        "NAUTOBOT_VER": context.ansible_filters.nautobot_ver,
        "PYTHON_VER": context.ansible_filters.python_ver,
    }
    compose_command = f'docker-compose --project-name {context.ansible_filters.project_name} --project-directory "{context.ansible_filters.compose_dir}"'
    for compose_file in context.ansible_filters.compose_files:
        compose_file_path = os.path.join(context.ansible_filters.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.ansible_filters.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.ansible_filters.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations ansible_filters"
